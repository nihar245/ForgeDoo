  ...
  const filteredOrders = orders.filter(order => {
    const matchesSearch = (order.product_name 
      || '').toLowerCase().includes(searchTerm.toLowerCase()) || 
                         (order.reference || `MO${order.id}`).toLowerCase().includes(searchTerm.toLowerCase())
    const mappedStatus = mapStatusForFilter(order.status)
    const matchesFilter = filterStatus === 'all' || mappedStatus === filterStatus
    return matchesSearch && matchesFilter
  })

  const getComponentStatus = (order) => {
    // This would need to be implemented based on component availability API
    ...
    <div className="space-y-6">
            ...
            <tbody>
              {loading ? (
                ...
                filteredOrders.map((order, index) => (
                  ...
                  >
                    <td className="py-5 px-6 font-semibold text-slate-900 group-hover:text-blue-600 transition-colors">
                      {order.reference || `MO${order.id}`}
                    </td>
                    ...
                    <td className="py-5 px-6">
                      <span className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold ring-1 bg-slate-50 text-slate-600 ring-slate-200">
                        {getComponentStatus(order)}
                      </span>
                    </td>
                    <td className="py-5 px-6 text-slate-700 font-semibold">{order.quantity}</td>
                    <td className="py-5 px-6 text-slate-600 font-medium">units</td>
                    <td className="py-5 px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="glass p-6 rounded-xl text-center border border-slate-200 hover:shadow-lg transition-all duration-200">
          ...
          <div className="text-sm text-slate-600 font-medium">Draft Orders</div>
        </div>
        <div className="glass p-6 rounded-xl text-center border border-slate-200 hover:shadow-lg transition-all duration-200">

...
import { manufacturingOrdersAPI, stockLedgerAPI } from '../../services'
    ...
    const fetchNotifications = async () => {
      try {
        ...
        const [ordersResponse, stockResponse] = await Promise.all([
          manufacturingOrdersAPI.getAll().catch(() => ({ data: [] })),
          stockLedgerAPI.getSummary().catch(() => ({ data: [] }))
        ])

        const recentNotifications = []

        // Add notifications for recent manufacturing orders
        if (ordersResponse.data) {
          const recentOrders = ordersResponse.data.slice(0, 2)
          recentOrders.forEach((order) => {
            recentNotifications.push({
              id: `mo-${order.id}`,
              message: `Manufacturing order ${order.reference || `#MO${order.id}`} ${order.status === 'done' ? 'completed' : 'updated'}`,
        ...
        if (stockResponse.data) {
          const lowStockItems = stockResponse.data.filter(item => (item.on_hand || 0) < 20).slice(0, 2)
          lowStockItems.forEach(item => {
            recentNotifications.push({
              id: `stock-${item.id}`,
              message: `Low stock alert: ${item.name || item.product_name} (${item.on_hand || 0} units remaining)`,
              time: '5 mins ago'
            })
          })
        }

        // Set notifications or keep empty if no real data
        ...
        // Keep notifications empty to show only real data
        setNotifications([])
      }
    }
          ...
          <div className="max-w-[100vw] mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="grid grid-cols-[auto_1fr_auto] items-center">
              ...
              <div className="justify-self-center flex items-center gap-3">
                <Link to={isAuthenticated ? "/dashboard" : "/"} className="inline-flex items-center">

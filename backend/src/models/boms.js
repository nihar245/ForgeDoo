...
export async function getBom(id){
  const bomR = await query('SELECT b.*, p.name as product_name FROM bom b LEFT JOIN products p ON p.id = b.product_id WHERE b.id=$1',[id]);
  if(!bomR.rowCount) return null;
  
  // Get components with product details
  const components = await query(`
    SELECT bc.*, p.name as product_name, p.uom, p.unit_cost 
  ...
  // Get operations with work center details
  const ops = await query(`
    SELECT bo.*, wc.name as work_center_name, wc.location as work_center_location
    FROM bom_operations bo 
    LEFT JOIN work_centers wc ON wc.id = bo.workcenter_id 
    WHERE bo.bom_id = $1 
    ORDER BY bo.sequence, bo.id
  `,[id]);

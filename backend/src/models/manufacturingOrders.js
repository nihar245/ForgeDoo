...
export async function listMOs({ status, product, created_by }){
  ...
  if(status){ clauses.push(`mo.status=$${i++}`); params.push(status); }
  if(product){ clauses.push(`mo.product_id=$${i++}`); params.push(product); }
  if(created_by){ clauses.push(`mo.created_by=$${i++}`); params.push(created_by); }
...
export async function listLateMOs({ created_by } = {}){
  ...
  if(created_by){ filter = `AND mo.created_by=$${i++}`; params.push(created_by); }
  const res = await query(`
    SELECT mo.*, p.name as product_name 
    FROM manufacturing_orders mo 
    LEFT JOIN products p ON mo.product_id = p.id 
    WHERE mo.end_date IS NOT NULL
      AND mo.end_date < CURRENT_DATE
      AND mo.status NOT IN ('not_assigned','to_close')
      ${filter}
    ORDER BY mo.end_date ASC` , params);
...
export async function createMO({ product_id, quantity, start_date, end_date, created_by, assignee_id, bom_id }){
  ...
  // Get the product name for the newly created MO
  const productRes = await query('SELECT name FROM products WHERE id=$1',[product_id]);
  const product_name = productRes.rowCount ? productRes.rows[0].name : null;
  
  return { ...mo, product_name, reference: formatMo(mo.id), is_late: computeLate(mo), is_unassigned: !mo.assignee_id };
}
...
export async function attachBOM(id, bom_id){
  ...
  return { ...mo, product_name, reference: formatMo(mo.id) };
}

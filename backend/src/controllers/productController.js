import Joi from 'joi';
import { listProducts, getProduct, createProduct, updateProduct, deleteProduct, listFinishedProducts, listRawMaterials, listByCategory } from '../models/products.js';
  ...
  try {
    ...
    if (category) {
      ...
      if (category === 'finished') {
        items = await listFinishedProducts();
      } else if (category === 'raw_material') {
        items = await listRawMaterials();
      } else {
        items = await listByCategory(category);
      }
    } else {
